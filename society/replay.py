...
def render_generation_timeline(storage: StorageManager, generation_id: int) -> str:
    ...
    if role_variant_count:
        ...
        for role in summary.get("selection_summary", {}).get("bundle_archive_proving_roles", []):
        ...
        for role in summary.get("selection_summary", {}).get("bundle_archive_reentry_backoff_roles", []):
        ...
        lines.append(
            f"  newly_admitted_count:{summary.get('selection_summary', {}).get('newly_admitted_count', 0)}"
        )
        lines.append(
            "  post_admission_grace_count:"
        ...
        lines.append(
            f"  archive_reentry_converted_count:{summary.get('selection_summary', {}).get('archive_reentry_converted_count', 0)}"
        )
        lines.append(
            "  archive_reentry_mean_gap_generations:"
            f"{summary.get('selection_summary', {}).get('archive_reentry_mean_gap_generations', 0.0)}"
        )
        lines.append(
        ...
        for item in summary.get("selection_summary", {}).get("archive_proving_bundles", []):
            lines.append(
                "  admission_proving:"
                f"{item['role']}:{item['bundle_signature']}"
                f" candidate_generations={item['archive_candidate_generations']}"
        ...
        for item in summary.get("selection_summary", {}).get("post_admission_grace_bundles", []):
            lines.append(
                "  admission_grace:"
                f"{item['role']}:{item['bundle_signature']}"
        ...
        for item in summary.get("selection_summary", {}).get("archive_reentry_block_bundles", []):
            lines.append(
                "  reentry_blocked:"
                f"{item['role']}:{item['bundle_signature']}"
